...
Read from Hive immediately
Show login screen with "auto-login" if remember_me=true OR show home if session valid
Trigger background session validation API call
If 200 OK + new data: Update UI silently, update Hive
If 304 Not Modified: Keep UI as-is
If 401: Clear session, show login screen
Scenario 3: App Restart No Internet (12h old cache)
Read from Hive immediately
Auto-login if session not expired by timestamp
API call fails (network error)
Fail silently, use cached data
Show non-blocking "offline mode" indicator
Retry on network reconnection (connectivity listener)
...
Read from Hive
Show UI with visible "data may be outdated" warning banner
Trigger API call immediately (not background)
If 200 OK: Update UI, remove warning, update Hive
If API fails: Keep showing stale data + warning
Auto-retry every 30 seconds with exponential backoff (max 5 minutes)
...
Flow Progression:
Screen 1: Personal Details → Next (validate + save state)
Screen 2: Professional Details → Next (validate + save state)
Screen 3: Address Details → Next (validate + save state)
Screen 4: Document Uploads → Next (validate + save state)
Screen 5: Payment → Submit (trigger final API call)
...
On screen exit: Save current screen data to Hive
...
Yes: Load all reg_* keys from Hive, navigate to reg_current_step
No: Clear all reg_* keys, start fresh from Screen 1
